...
x = pd.get_dummies(np.array(x).reshape(-1, )).values.reshape(15, 5, 31)
...
x_question = to_categorical(x_question, num_classes = 31)
...
model.add(Conv1D(32, 2, input_shape = (5, 31), padding = 'same'))
model.add(Conv1D(32, 2, input_shape = (5, 31), padding = 'same'))
